...
- 导出由**用户点击顶部「导出 MP4」按钮**完成（WebCodecs 硬编，产物自动存 `/vedio/exports/{标题}.mp4` 并触发浏览器下载；AI 不触发导出）
...
8. 完成：提醒用户点击页面顶部「导出 MP4」按钮自行导出（导出由用户操作，AI 不触发）
...
8. **结果反馈**：操作完成后一两句话向用户总结（时长、场景数、效果亮点）；完成后提醒用户可点顶部「导出 MP4」自行导出
