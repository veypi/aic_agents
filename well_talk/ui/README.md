...
纯 JavaScript 三维井筒可视化引擎（ES Module），零依赖构建、可直接在 HTML 页面中引用，仅依赖 three.js
...
> - 不再使用 importmap：`lib/well3d.js` 与 `lib/vendor/OrbitControls.js` 中的裸导入 `'three'` 已改为相对路径
...
> **技术文档**：从数据进入到解析、几何生成再到 Three.js 渲染的完整管线说明见 [`../docs/well3d-pipeline.md`](../docs/well3d-pipeline.md)。
...
<script type="module">
  import { Well3DViewer, DataStore, WellMap } from './lib/well3d.js';

  const store = new DataStore();
  store.set('position-logs', 'A-1', poslog);   // Float32Array，见下方数据格式
  store.set('casings', 'A-1', casings);

  const viewer = new Well3DViewer(document.getElementById('viewer'), { store });
...
| 功能 | 说明 |
|---|---|
| Wellbore trajectories | 线（radius=0）与圆管两种轨迹，弦长参数化样条插值（curve-interpolator） |
...
**暂不支持**：Web Worker 数据管线（改为内存 `DataStore`，同步生成）、SDF 着色器文字标注
...
### `Well3DViewer(container, options)`
...
- **按需图层**：套管/完井、套管鞋、地层标记、地层柱、射孔、深度标注、
