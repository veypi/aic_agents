...
| `--index <N>` | 可选。起始页（1 基），省略为 1；仅无 --script 时生效 |
| `--script <JSON>` | 可选。讲解脚本：JSON 数组 `[{"id":"页id","voice":"讲解词","stay":毫秒}]`，**数组顺序即演讲顺序**，支持错序跳页（如 1→3→2）。项不带 voice = 纯播放演示（只跳页，停留 `stay` 毫秒（默认 2000））；带 voice = 转语音讲解（跳页 + 播音，播完自然进下一页）。页面位置优先用 `id`（稳定，非线性页重排不漂移），也可用 `index`（1 基） |

返回 `{ ok, presenting: true, start, total, path }`；--script 时额外返回 `script`（执行项数）、`voiced`（成功播放语音项数）、`skipped`（找不到页面而跳过的项）。

**与 open_ppt 的区别**：`open_ppt` 只在编辑态打开（用户可继续编辑）；`run_ppt` 打开后立即进入全屏演示（演示态）。**完成制作后用 `run_ppt` 向用户展示成果**；仅想确认内容、继续编辑或等待用户反馈时用 `open_ppt`。修改已有 PPT 后重跑 `run_ppt`（带 path）会先刷新文档再演示，无需先 open 再 run。

### 播放/演讲模式（run_ppt + --script）
...
[{"id":"slide-1","voice":"欢迎观看，首先看背景"},
 ...
 {"id":"slide-4","stay":5000}]                     # 纯展示且多停留：5s
```

- 适用场景：用户说「演示一下 / 看看效果 / 给我讲讲 / 讲解一下」时，均用本命令；脚本内每项按需决定是否带语音，没有模式区分
- 停留时长不同页可不同：重点展示页给长一些（如 5s），过渡页短一些；无语音的项不会产生 TTS 费用
- **页面位置优先用 `id`**（页面稳定标识，非线性页面重排/编辑后不漂移）；也可用 `index`（1 基数字）
- **语音流水线**：带 voice 的项按序调用 speech 接口合成，**TTS 限流 3s/个**（阿里 Throttling.RateQuota）——请求间隔 3s 串行发出，合成完成时间不影响节流节奏；**第一个合成完即可开播**，播放环环按序逐项播放（每项播完才进下一项，前一项播放期间后续语音在后台合成）
- **语音失败不中断演讲**：某项合成失败（限流/超时等）该项停留 `stay` 毫秒（默认 2000）后继续，其余项照常；不要因单个失败重试整段语音
- 找不到的页跳过（返回 `skipped` 列表），不中断演示
- 每项讲解词口语化、分句自然，1~3 句为宜，避免超长无停顿句子（语音可带降调节奏）
- 语音播放不阻塞用户手动翻页，演示快捷键（S 演讲者视图等）仍可用；用户手动翻页后脚本依然按原序继续后续项
