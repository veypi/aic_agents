 ...
 *
 * The document format is a JSON object (bento/slides-compatible subset):
 *   { format, version, docId, title, size:{width,height}, theme:{...},
...
const INLINE_TAGS = { B: 1, STRONG: 1, I: 1, EM: 1, U: 1, S: 1, BR: 1, SPAN: 1, DIV: 1, P: 1, CODE: 1, SMALL: 1, SUB: 1, SUP: 1 }
const STYLE_OK = { color: 1, 'background-color': 1, 'font-weight': 1, 'font-style': 1, 'text-decoration': 1, 'text-decoration-line': 1 }
    ...
    if (n.nodeType === 1) {
      if (!INLINE_TAGS[n.tagName]) {
        ...
        if (n.tagName === 'SPAN' && style) {
          const kept = style.split(';').map(function (d) {
            const p = d.split(':'); return p.length === 2 && STYLE_OK[p[0].trim().toLowerCase()] ? d.trim() : ''
          }).filter(Boolean).join(';')
...
const DEFAULT_PALETTE = ['#3a6ff7', '#f0773a', '#3abf7c', '#b04ad9', '#d9a53a', '#3ab0c9']
...
function starterDoc() {
  ...
  s1.transition = 'fade'
  s1.elements.push({
    id: 'title', type: 'text', x: 80, y: 200, w: 800, h: 90, rotation: 0, opacity: 1,
    html: 'Presentation', fontSize: 64, fontFamily: theme.fontFamily, fontWeight: 700,
    color: theme.color, align: 'center', valign: 'middle', lineHeight: 1.1
  }, {
    id: 'sub', type: 'text', x: 180, y: 310, w: 600, h: 44, rotation: 0, opacity: 1,
    html: 'Edit me, then press Present', fontSize: 22, fontFamily: theme.fontFamily,
    fontWeight: 400, color: '#6b7280', align: 'center', valign: 'middle', lineHeight: 1.3
  })
  const s2 = blankSlide(theme)
  s2.elements.push({
    id: 'title', type: 'text', x: 60, y: 40, w: 600, h: 60, rotation: 0, opacity: 1,
    html: 'Second slide', fontSize: 40, fontFamily: theme.fontFamily, fontWeight: 700,
    color: theme.color, align: 'left', valign: 'middle', lineHeight: 1.1
  }, {
    id: 'box', type: 'shape', shape: 'rect', x: 60, y: 150, w: 220, h: 140, rotation: 0,
    opacity: 1, fill: theme.accent, stroke: '', strokeWidth: 0, radius: 12
  })
  return {
    format: 'ppt/1',
    version: 1,
...
function normalizeDoc(input) {
  ...
  doc.size = doc.size && doc.size.width ? doc.size : { width: 960, height: 540 }
  doc.theme = Object.assign(defaultTheme(), doc.theme || {})
  if (!doc.theme.chartPalette || !doc.theme.chartPalette.length) doc.theme.chartPalette = DEFAULT_PALETTE.slice()
  doc.slides = Array.isArray(doc.slides) && doc.slides.length ? doc.slides : [blankSlide(doc.theme)]
  doc.slides.forEach(function (s) {
    ...
    s.elements.forEach(function (el) {
      el.id = el.id || uid()
      el.x = +el.x || 0; el.y = +el.y || 0
      el.w = +el.w || 100; el.h = +el.h || 60
...
function resolveAsset(doc, ref) {
  return typeof ref === 'string' && ref.indexOf('asset:') === 0
    ? ((doc.assets || {})[ref.slice(6)] || '') : ref
}

function morphKeyOf(el) { return el.morphId || el.id }

/* -------------------------------------------------------- element render */

function baseBoxCss(el) {
  let s = 'left:' + el.x + 'px;top:' + el.y + 'px;width:' + el.w + 'px;height:' + el.h + 'px;'
  ...
  if (el.shadow) {
    ...
    arr.forEach(function (sh) {
      filt.push('drop-shadow(' + (sh.x || 0) + 'px ' + (sh.y || 0) + 'px ' + (sh.blur || 8) + 'px ' + (sh.color || 'rgba(0,0,0,.3)') + ')')
    })
  }
  if (el.blur) filt.push('blur(' + el.blur + 'px)')
  if (filt.length) s += 'filter:' + filt.join(' ') + ';'
  if (el.blend) s += 'mix-blend-mode:' + el.blend + ';'
  if (el.backdropFilter) s += 'backdrop-filter:blur(' + el.backdropFilter + 'px);-webkit-backdrop-filter:blur(' + el.backdropFilter + 'px);'
  return s
}

function gradientCss(g, fallback) {
  if (!g || !g.stops || !g.stops.length) return fallback || 'none'
  const stops = g.stops.map(function (st) { return st.color + ' ' + Math.round((st.at || 0) * 100) + '%' }).join(',')
  return 'linear-gradient(' + (g.angle == null ? 90 : g.angle) + 'deg,' + stops + ')'
}

let gradientSeq = 0
function svgFillAttrs(el) {
  // returns {fill, defs} — defs carries a linearGradient when fillGradient set
  if (el.fillGradient && el.fillGradient.stops && el.fillGradient.stops.length) {
    const gid = 'pptg' + (++gradientSeq)
    const stops = el.fillGradient.stops.map(function (st) {
      return '<stop offset="' + ((st.at || 0) * 100) + '%" stop-color="' + escapeHtml(st.color) + '"/>'
    }).join('')
    return {
      fill: 'url(#' + gid + ')',
      defs: '<defs><linearGradient id="' + gid + '" gradientTransform="rotate(' + ((el.fillGradient.angle || 0) - 90) + ' 0.5 0.5)">' + stops + '</linearGradient></defs>'
    }
  }
...
function strokeAttrs(el) {
  let s = 'stroke="' + (el.stroke || 'none') + '" stroke-width="' + (el.strokeWidth || 0) + '"'
  const style = el.strokeStyle
  if (style === 'dashed') s += ' stroke-dasharray="' + ((el.strokeWidth || 2) * 3) + ' ' + ((el.strokeWidth || 2) * 2) + '"'
  else if (style === 'dotted') s += ' stroke-dasharray="0 ' + ((el.strokeWidth || 2) * 2.2) + '" stroke-linecap="round"'
  else if (el.strokeDash) s += ' stroke-dasharray="' + el.strokeDash + ' ' + el.strokeDash + '"'
  return s
}
...
function lineMarkers(el, sw) {
  ...
  ;['start', 'end'].forEach(function (which) {
    ...
    let body = ''
    if (k === 'arrow') body = '<path d="M0,0L10,5L0,10Z" fill="' + escapeHtml(el.stroke || '#000') + '"/>'
    else if (k === 'dot') body = '<circle cx="5" cy="5" r="4" fill="' + escapeHtml(el.stroke || '#000') + '"/>'
    else if (k === 'bar') body = '<rect x="4" y="0" width="2.4" height="10" fill="' + escapeHtml(el.stroke || '#000') + '"/>'
    defs += '<marker id="' + mid + '" viewBox="0 0 10 10" refX="' + (which === 'end' ? 9 : 1) + '" refY="5" markerWidth="8" markerHeight="8" orient="auto-start-reverse" markerUnits="strokeWidth">' + body + '</marker>'
...
function renderShape(el) {
  ...
  if (kind === 'rect') {
    ...
    body = '<ellipse cx="' + w / 2 + '" cy="' + h / 2 + '" rx="' + Math.max(w / 2 - pad, 0.1) + '" ry="' + Math.max(h / 2 - pad, 0.1) + '" fill="' + f.fill + '" ' + stroke + '/>'
  } else if (kind === 'triangle') {
    body = '<polygon points="' + w / 2 + ',' + pad + ' ' + (w - pad) + ',' + (h - pad) + ' ' + pad + ',' + (h - pad) + '" fill="' + f.fill + '" ' + stroke + '/>'
  } else if (kind === 'arrow') {
    const a = h * 0.32, b = h * 0.68, head = Math.min(w * 0.45, h * 0.9)
    body = '<polygon points="0,' + a + ' ' + (w - head) + ',' + a + ' ' + (w - head) + ',0 ' + w + ',' + h / 2 + ' ' + (w - head) + ',' + h + ' ' + (w - head) + ',' + b + ' 0,' + b + '" fill="' + f.fill + '" ' + stroke + '/>'
  } else if (kind === 'line') {
    const m = lineMarkers(el, sw)
    body = '<line x1="' + pad + '" y1="' + h / 2 + '" x2="' + (w - pad) + '" y2="' + h / 2 + '" ' + stroke + m.attrs + '/>'
    return '<svg width="100%" height="100%" viewBox="' + viewBox + '" preserveAspectRatio="none"><defs>' + m.defs + '</defs>' + body + '</svg>'
  } else if (kind === 'path' && el.d) {
    const m2 = lineMarkers(el, sw)
    const vb = el.pathBox ? el.pathBox.join(' ') : viewBox
    return '<svg width="100%" height="100%" viewBox="' + vb + '" preserveAspectRatio="none"><defs>' + m2.defs + '</defs><path d="' + escapeHtml(el.d) + '" fill="' + f.fill + '" ' + stroke + m2.attrs + '/></svg>'
  }
  return '<svg width="100%" height="100%" viewBox="' + viewBox + '" preserveAspectRatio="none">' + f.defs + body + '</svg>'
}
...
function renderChart(el, doc) {
  ...
  const pal = (opt.color && opt.color.length ? opt.color : null) || doc.theme.chartPalette || DEFAULT_PALETTE
  const W = Math.max(el.w, 60), H = Math.max(el.h, 60)
  let out = ''
  const type = (series[0] && series[0].type) || 'bar'

  if (type === 'pie') {
    const data = (series[0] && series[0].data) || []
    const vals = data.map(function (d) { return typeof d === 'object' && d !== null ? +d.value || 0 : +d || 0 })
    const total = vals.reduce(function (a, b) { return a + b }, 0) || 1
    const hasNames = data.some(function (d) { return d && typeof d === 'object' && d.name })
    const cx = hasNames ? W * 0.38 : W / 2, cy = H / 2, r = Math.min(hasNames ? W * 0.6 : W, H) / 2 - 8
    let a0 = -Math.PI / 2
    vals.forEach(function (v, i) {
      ...
      out += '<path d="M' + cx + ',' + cy + 'L' + (cx + r * Math.cos(a0)) + ',' + (cy + r * Math.sin(a0)) +
        'A' + r + ',' + r + ' 0 ' + large + ' 1 ' + (cx + r * Math.cos(a1)) + ',' + (cy + r * Math.sin(a1)) + 'Z" fill="' + pal[i % pal.length] + '"/>'
      a0 = a1
    })
    if (hasNames) {
      data.forEach(function (d, i) {
        const y = H / 2 - (data.length - 1) * 9 + i * 18
        out += '<rect x="' + (W * 0.68) + '" y="' + (y - 8) + '" width="10" height="10" rx="2" fill="' + pal[i % pal.length] + '"/>' +
          '<text x="' + (W * 0.68 + 16) + '" y="' + y + '" font-size="11" fill="#555">' + escapeHtml(d.name || '') + '</text>'
      })
    }
  } else {
    const cats = (opt.xAxis && opt.xAxis.data) || []
    const padL = 38, padB = 24, padT = series.some(function (s) { return s.name }) && series.length > 1 ? 22 : 10, padR = 10
    const iw = W - padL - padR, ih = H - padT - padB
    const all = []
    series.forEach(function (s) { (s.data || []).forEach(function (v) { all.push(+v || 0) }) })
    ...
    const Y = function (v) { return padT + ih * (1 - (v - min) / span) }
    // gridlines + y labels
    for (let g = 0; g <= 4; g++) {
      const gv = min + span * g / 4, gy = Y(gv)
      out += '<line x1="' + padL + '" y1="' + gy + '" x2="' + (padL + iw) + '" y2="' + gy + '" stroke="rgba(128,128,128,.18)"/>' +
        '<text x="' + (padL - 5) + '" y="' + (gy + 3.5) + '" font-size="10" fill="#888" text-anchor="end">' + fmtNum(gv) + '</text>'
    }
    // x labels
    for (let c = 0; c < n; c++) {
      const cxp = padL + iw * (c + 0.5) / n
      out += '<text x="' + cxp + '" y="' + (H - 7) + '" font-size="10" fill="#888" text-anchor="middle">' + escapeHtml(cats[c] != null ? cats[c] : '') + '</text>'
    }
    // axes
    out += '<line x1="' + padL + '" y1="' + padT + '" x2="' + padL + '" y2="' + (padT + ih) + '" stroke="rgba(128,128,128,.4)"/>' +
      '<line x1="' + padL + '" y1="' + Y(Math.max(min, 0)) + '" x2="' + (padL + iw) + '" y2="' + Y(Math.max(min, 0)) + '" stroke="rgba(128,128,128,.4)"/>'
    const band = iw / n
    series.forEach(function (s, si) {
      const col = pal[si % pal.length]
      const vals2 = (s.data || []).map(function (v) { return +v || 0 })
      if (s.type === 'line') {
        const pts = vals2.map(function (v, i) { return (padL + band * (i + 0.5)) + ',' + Y(v) }).join(' ')
        out += '<polyline points="' + pts + '" fill="none" stroke="' + col + '" stroke-width="2.4" stroke-linejoin="round"/>'
        vals2.forEach(function (v, i) { out += '<circle cx="' + (padL + band * (i + 0.5)) + '" cy="' + Y(v) + '" r="3" fill="' + col + '"/>' })
      } else if (s.type === 'scatter') {
        vals2.forEach(function (v, i) { out += '<circle cx="' + (padL + band * (i + 0.5)) + '" cy="' + Y(v) + '" r="4" fill="' + col + '" fill-opacity=".85"/>' })
      } else { // bar
        ...
        vals2.forEach(function (v, i) {
          ...
          out += '<rect x="' + x + '" y="' + y0 + '" width="' + Math.max(bw - 2, 1) + '" height="' + Math.max(y1 - y0, 0.5) + '" rx="1.5" fill="' + col + '"/>'
        })
      }
    })
    if (padT === 22) { // legend
      series.forEach(function (s, si) {
        const lx = padL + si * 90
        out += '<rect x="' + lx + '" y="6" width="10" height="10" rx="2" fill="' + pal[si % pal.length] + '"/>' +
          '<text x="' + (lx + 15) + '" y="15" font-size="10" fill="#777">' + escapeHtml(s.name || '') + '</text>'
      })
    }
  }
...
function renderTable(el) {
  var st = el.style || {}
  var cols = (el.columns || []).map(function (c) { return '<col style="width:' + (c.w * 100) + '%">' }).join('')
  var rows = (el.rows || []).map(function (row, ri) {
    var isHead = el.header && ri === 0
    var tag = isHead ? 'th' : 'td'
    var tds = (row.cells || []).map(function (cell, ci) {
      var cs = 'text-align:' + (cell.align || 'left') + ';'
      cs += 'color:' + (cell.color || (isHead ? st.headerColor || '#fff' : st.color || '#1f2430')) + ';'
  ...
  var ts = 'width:100%;height:100%;border-collapse:collapse;table-layout:fixed;' +
  ...
  return '<table style="' + ts + '">' + (cols ? '<colgroup>' + cols + '</colgroup>' : '') + rows + '</table>'
}

/** Resolve {{page}}, {{pages}}, {{title}} tokens in text HTML. */
function resolveFields(html, ctx) {
  if (!ctx || html.indexOf('{{') < 0) return html
  var pad = function (n, arg) { var w = parseInt(arg || '', 10); return w > 0 ? String(n).padStart(w, '0') : String(n) }
  return html.replace(/\{\{\s*(page|pages|title)(?::([^}]*))?\s*\}\}/gi, function (_m, name, arg) {
  ...
  if (el.type === 'text') {
    ...
    if (!html && el.placeholder) {
      if (ctx.hidePlaceholders || ctx.present) {
        ...
        node.innerHTML = '<span class="ppt-placeholder">' + escapeHtml(el.placeholder) + '</span>'
    ...
    img.draggable = false
    img.alt = ''
    img.style.cssText = 'width:100%;height:100%;display:block;object-fit:' + (el.fit || 'contain') + ';' +
      (el.radius ? 'border-radius:' + el.radius + 'px;' : '')
    node.appendChild(img)
    node.style.overflow = 'hidden'
    if (el.radius) node.style.borderRadius = el.radius + 'px'
  } else if (el.type === 'svg') {
    var markup = el.asset ? resolveAsset(doc, 'asset:' + el.asset) : (el.markup || '')
    node.innerHTML = (el.css ? '<style>' + el.css.replace(/<\//g, '<\\/') + '</style>' : '') + markup
...
function renderSlide(slide, doc, ctx) {
  ...
  if (!ctx.fields) {
    ...
    ctx.fields = {
      page: upto.filter(function (s) { return !s.stateOf }).length,
      pages: doc.slides.filter(function (s) { return !s.stateOf }).length,
      title: doc.title
    }
  }
...
function injectDocFonts(doc) {
  if (!doc.fonts || !doc.fonts.length) return
  var id = 'ppt-fonts-' + doc.docId
  if (document.getElementById(id)) return
  var css = doc.fonts.map(function (f) {
    var src = resolveAsset(doc, 'asset:' + f.asset)
    if (!src) return ''
    return '@font-face{font-family:"' + f.family + '";src:url(' + src + ');' +
      (f.weight ? 'font-weight:' + f.weight + ';' : '') + (f.style ? 'font-style:' + f.style + ';' : '') + '}'
  }).join('')
  if (!css) return
...
var easeInOutQuad = function (p) { return p < 0.5 ? 2 * p * p : 1 - Math.pow(-2 * p + 2, 2) / 2 } // power2.inOut
var easeOutQuad = function (p) { return 1 - (1 - p) * (1 - p) } // power2.out
...
var NAMED_COLORS = { white: '#ffffff', black: '#000000', red: '#ff0000', green: '#008000', blue: '#0000ff', yellow: '#ffff00', orange: '#ffa500', purple: '#800080', gray: '#808080', grey: '#808080' }
...
function colorParts(v) {
  ...
  if (/^#[0-9a-fA-F]{3}$/.test(v)) v = '#' + v.slice(1).split('').map(function (c) { return c + c }).join('')
...
function lerpColor(a, b, p) {
  return rgbaStr([a[0] + (b[0] - a[0]) * p, a[1] + (b[1] - a[1]) * p, a[2] + (b[2] - a[2]) * p, a[3] + (b[3] - a[3]) * p])
}

/** Colour of a gradient at position t (piecewise-linear between stops). */
function sampleGradient(stops, t) {
  var s = stops.slice().sort(function (x, y) { return x.at - y.at })
  if (t <= s[0].at) return rgbaStr(colorParts(s[0].color) || [0, 0, 0, 0])
  for (var i = 0; i < s.length - 1; i++) {
    var a = s[i], b = s[i + 1]
    if (t <= b.at) {
      var f = b.at === a.at ? 0 : (t - a.at) / (b.at - a.at)
      var ca = colorParts(a.color) || [0, 0, 0, 0], cb = colorParts(b.color) || [0, 0, 0, 0]
...
function chartSeries(opt) { return (opt && Array.isArray(opt.series)) ? opt.series : [] }
...
function chartsCompatible(oa, ob) {
  var sa = chartSeries(oa), sb = chartSeries(ob)
  if (!sa.length || sa.length !== sb.length) return false
  var ca = (oa && oa.xAxis && oa.xAxis.data) || [], cb = (ob && ob.xAxis && ob.xAxis.data) || []
...
var ENTER_DEFAULT_DUR = { 'fade-up': 0.55, 'fade': 0.55, 'fade-down': 0.55, 'slide-left': 0.75, 'slide-right': 0.75, 'slide-up': 0.75, 'slide-down': 0.75 }
...
Presenter.prototype.open = function (startIdx) {
  ...
  ov.querySelector('.ppt-present-progress').style.display = showProgress ? '' : 'none'
...
Presenter.prototype.close = function () {
  ...
  this.tip = null
  this.black = null
  var vids = document.querySelectorAll('.ppt-present video, .ppt-present audio')
...
Presenter.prototype.toggleSpeaker = function () {
  if (this.speakerWin) { this.closeSpeaker(); return }
  var w = window.open('', 'ppt-speaker', 'width=460,height=620')
  if (!w) return
  this.speakerWin = w
  this.speakerT0 = Date.now()
  var self = this
  this._spTimer = setInterval(function () { self.paintSpeaker() }, 1000)
  w.addEventListener('unload', function () {
    if (self._spTimer) { clearInterval(self._spTimer); self._spTimer = null }
...
Presenter.prototype.closeSpeaker = function () {
  if (this._spTimer) { clearInterval(this._spTimer); this._spTimer = null }
...
function slideFirstText(slide) {
  for (var i = 0; i < slide.elements.length; i++) {
    ...
    if (el.type === 'text' && el.html) {
      var t = String(el.html).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim()
...
Presenter.prototype.paintSpeaker = function () {
  ...
  if (!w) return
  if (w.closed) { this.closeSpeaker(); return }
  var slide = this.slide
  if (!slide) return
  var lin = this.linear()
  var cur = slide
  if (cur.stateOf) {
    var p = this.doc.slides.find(function (s) { return s.id === cur.stateOf })
    if (p) cur = p
  }
  var pos = lin.indexOf(cur)
  if (pos < 0) pos = 0
  var nextSlide = lin[pos + 1]
  ...
  var notes = (slide.notes || '').replace(/</g, '&lt;') || '<span style="color:#666">(no notes)</span>'
  ...
  w.document.body.innerHTML =
    ...
    '<div style="font-size:32px;font-weight:700;font-variant-numeric:tabular-nums">' + mm + ':' + ss + '</div>' +
    '<div style="color:#9aa0ad">' + (pos + 1) + ' / ' + lin.length + (slide.stateOf ? ' \u00B7 state' : '') + '</div>' +
    '</div>' +
    '<div style="flex:1;min-height:0;overflow:auto;background:#1c1f27;border-radius:10px;padding:14px;line-height:1.7;white-space:pre-wrap">' + notes + '</div>' +
    '<div style="color:#9aa0ad;font-size:12px">NEXT</div>' +
    '<div style="background:#1c1f27;border-radius:10px;padding:12px;color:#c6cad4;min-height:44px">' + (nextText.replace(/</g, '&lt;') || '(blank)') + '</div>' +
    '</div>'
}
...
Presenter.prototype.toggleBlack = function () {
  if (this.black) { this.black.remove(); this.black = null; return }
  if (!this.overlay) return
...
Presenter.prototype.fit = function () {
  ...
  var w = this.doc.size.width, h = this.doc.size.height
...
Presenter.prototype.linear = function () {
  return this.doc.slides.filter(function (s) { return !s.stateOf })
}

Presenter.prototype.updateChrome = function () {
  var lin = this.linear()
  var cur = this.slide
  var pos = cur.stateOf ? lin.indexOf(this.doc.slides.find(function (s) { return s.id === cur.stateOf })) : lin.indexOf(cur)
  if (pos < 0) pos = 0
...
Presenter.prototype.applyEnterFx = function (layer, slide) {
  ...
  slide.elements.forEach(function (el) {
    ...
    if (el.fx && el.fx.enter) {
      ...
      node.style.animation = 'ppt-enter-' + el.fx.enter + ' ' + dur + 's cubic-bezier(.22,.8,.36,1) ' + delay + 's both'
    }
...
Presenter.prototype.animateNumbers = function (node, delay, dur) {
  ...
  targets.forEach(function (tn) {
    var re = /\d[\d,.]*/g, m, last = 0, frag = null
    while ((m = re.exec(tn.nodeValue))) {
      if (!frag) frag = document.createDocumentFragment()
      frag.appendChild(document.createTextNode(tn.nodeValue.slice(last, m.index)))
  ...
  node.querySelectorAll('[data-final]').forEach(function (sp) {
    ...
    if (isNaN(num)) { sp.textContent = raw; return }
    var dec = (raw.split('.')[1] || '').length
    var grouped = raw.indexOf(',') >= 0
    self.tweens.push(tween({
      duration: dur, delay: delay, ease: easeOutQuad,
      update: function (p) {
        var v = num * p
        sp.textContent = grouped ? Math.round(v).toLocaleString('en-US') : (dec ? v.toFixed(dec) : String(Math.round(v)))
      },
      complete: function () { sp.textContent = raw }
    }))
  })
}
...
Presenter.prototype.onHoverMove = function (e) {
  ...
  var g = ''
  if (node) {
    var el = slide.elements.find(function (x) { return x.id === node.getAttribute('data-el-id') })
    g = (el && el.group) || ''
  }
...
Presenter.prototype.applyHover = function (slide, g) {
  ...
  slide.elements.forEach(function (el) {
    ...
    if (hv.type === 'focus-group') {
      node.style.opacity = (!g || el.group === g) ? el.opacity : el.opacity * (hv.dim == null ? 0.25 : hv.dim)
    } else if (hv.type === 'reveal') {
      var set = g || hv.default || ''
      node.style.opacity = (!el.showOnHover || el.showOnHover === set) ? el.opacity : 0
    }
  })
}
...
Presenter.prototype.chartTip = function (e) {
  ...
  var slide = this.slide
  if (!node || !slide) { tip.style.display = 'none'; return }
  var el = slide.elements.find(function (x) { return x.id === node.getAttribute('data-el-id') })
  if (!el || el.type !== 'chart') { tip.style.display = 'none'; return }
  var opt = el.option || {}
  var series = chartSeries(opt)
  if (!series.length) { tip.style.display = 'none'; return }
  var type = series[0].type || 'bar'
  var pal = (opt.color && opt.color.length ? opt.color : null) || this.doc.theme.chartPalette || DEFAULT_PALETTE
  ...
  if (type === 'pie') {
    ...
    var hasNames = data.some(function (d) { return d && typeof d === 'object' && d.name })
    var pcx = hasNames ? W * 0.38 : W / 2, pcy = H / 2
    var rad = Math.sqrt((mx - pcx) * (mx - pcx) + (my - pcy) * (my - pcy))
    var rMax = Math.min(hasNames ? W * 0.6 : W, H) / 2 - 8
    if (rad > rMax + 10) { tip.style.display = 'none'; return }
    var ang = Math.atan2(my - pcy, mx - pcx)
    var vals = data.map(function (d) { return typeof d === 'object' && d !== null ? +d.value || 0 : +d || 0 })
    var total = vals.reduce(function (a, b) { return a + b }, 0) || 1
    var acc = -Math.PI / 2, idx = -1
    for (var i = 0; i < vals.length; i++) {
      var nxt = acc + vals[i] / total * Math.PI * 2
      if (ang >= acc && ang < nxt) { idx = i; break }
      acc = nxt
    }
    if (idx < 0 || vals[idx] <= 0) { tip.style.display = 'none'; return }
    var d = data[idx]
    var nm = typeof d === 'object' && d !== null ? (d.name || '') : ''
    html = '<i style="background:' + pal[idx % pal.length] + '"></i>' + escapeHtml(nm) + ' <b>' + vals[idx] + '</b> (' + Math.round(vals[idx] / total * 100) + '%)'
  } else {
    var cats = (opt.xAxis && opt.xAxis.data) || []
    if (!cats.length) { tip.style.display = 'none'; return }
    var padL = 38, padR = 10
    var n = Math.max(cats.length, 1)
    var idx2 = Math.floor((mx - padL) / ((W - padL - padR) / n))
    if (idx2 < 0 || idx2 >= cats.length) { tip.style.display = 'none'; return }
    html = '<div class="t">' + escapeHtml(String(cats[idx2])) + '</div>' + series.map(function (s, si) {
      var v = (s.data || [])[idx2]
      if (v && typeof v === 'object') v = v.value
      return '<div><i style="background:' + pal[si % pal.length] + '"></i>' + escapeHtml(s.name || 'S' + (si + 1)) + ' <b>' + (v == null ? '—' : v) + '</b></div>'
    }).join('')
  }
  tip.innerHTML = html
  tip.style.display = ''
...
Presenter.prototype.show = function (idx, dir, instant) {
  ...
  this.killTweens()
  var oldLayer = this.layer, oldSlide = this.slide
  var newLayer = renderSlide(slide, this.doc, { present: true, hidePlaceholders: true })
  ...
  function finishOld() { if (oldLayer && oldLayer.parentNode) oldLayer.remove() }
  ...
  if (transition === 'slide') {
    var d = dir >= 0 ? 1 : -1
    newLayer.animate(
      [{ transform: 'translateX(' + (100 * d) + '%)' }, { transform: 'translateX(0)' }],
      { duration: D, easing: 'cubic-bezier(.22,.8,.36,1)' })
    var out = oldLayer.animate(
      [{ transform: 'translateX(0)', opacity: 1 }, { transform: 'translateX(' + (-40 * d) + '%)', opacity: 0.4 }],
      { duration: D, easing: 'cubic-bezier(.22,.8,.36,1)' })
    ...
    newLayer.animate(
      ...
      { duration: D, easing: 'cubic-bezier(.22,.8,.36,1)' })
    var zo = oldLayer.animate([{ opacity: 1 }, { opacity: 0 }], { duration: D * 0.7 })
    zo.onfinish = finishOld
  } else if (transition === 'fade') {
    newLayer.animate([{ opacity: 0 }, { opacity: 1 }], { duration: D * 0.75, easing: 'ease' })
    var fo = oldLayer.animate([{ opacity: 1 }, { opacity: 0 }], { duration: D * 0.75, easing: 'ease' })
...
Presenter.prototype.runMorph = function (newLayer, oldSlide, newSlide) {
  var self = this
  var fromMap = {}
  oldSlide.elements.forEach(function (el) { fromMap[morphKeyOf(el)] = el })
  ...
  newSlide.elements.forEach(function (b) {
    ...
    if (a.type === 'chart' && b.type === 'chart') {
      ...
      self.tweens.push(tween({
        duration: D, ease: easeInOutQuad,
        update: function (p) {
          var x = a.x + (b.x - a.x) * p
          var y = a.y + (b.y - a.y) * p
          var w = a.w + (b.w - a.w) * p
          var h = a.h + (b.h - a.h) * p
          var r = (a.rotation || 0) + ((b.rotation || 0) - (a.rotation || 0)) * p
    ...
    if (a.opacity !== b.opacity) {
      self.tweens.push(tween({
        duration: D, ease: easeInOutQuad,
        update: function (p) { node.style.opacity = a.opacity + (b.opacity - a.opacity) * p }
      }))
    }

    // text colour morph
    if (a.type === 'text' && b.type === 'text' && a.color !== b.color) {
      var ca = colorParts(a.color), cb = colorParts(b.color)
      if (ca && cb) {
        self.tweens.push(tween({
          duration: D, ease: easeInOutQuad,
          update: function (p) { node.style.color = lerpColor(ca, cb, p) },
          complete: function () { node.style.color = b.color || '' }
        }))
      }
    }

    // shape fill / stroke morph (solid ⇄ solid, or gradient involved)
    if (a.type === 'shape' && b.type === 'shape') self.morphShapeFill(node, a, b)
  })
...
Presenter.prototype.morphChart = function (node, a, b) {
  ...
  if (!compat) {
    ...
    ghost.style.cssText = 'position:absolute;left:0;top:0;width:100%;height:100%;pointer-events:none;'
    ghost.innerHTML = renderChart({ option: a.option, w: b.w, h: b.h }, this.doc)
    node.appendChild(ghost)
  }
  this.tweens.push(tween({
    duration: MORPH_DURATION, ease: easeInOutQuad,
    update: function (p) {
      var x = a.x + (b.x - a.x) * p, y = a.y + (b.y - a.y) * p
      var w = Math.max(a.w + (b.w - a.w) * p, 1), h = Math.max(a.h + (b.h - a.h) * p, 1)
      node.style.left = x + 'px'; node.style.top = y + 'px'
      node.style.width = w + 'px'; node.style.height = h + 'px'
      if (compat) {
        node.innerHTML = renderChart({ option: lerpChartOption(a.option, b.option, p), w: w, h: h }, self.doc)
      } else {
        ghost.style.opacity = 1 - p
      }
    },
    complete: function () {
      node.innerHTML = renderChart({ option: b.option, w: b.w, h: b.h }, self.doc)
...
Presenter.prototype.morphShapeFill = function (node, a, b) {
  var self = this
  var target = node.querySelector('svg rect, svg ellipse, svg polygon, svg line, svg path')
  if (!target) return
  var D = MORPH_DURATION

  function attrColorTween(el, attr, from, to, complete) {
    var ca = colorParts(from), cb = colorParts(to)
    if (!ca || !cb) { if (complete) complete(); return }
    self.tweens.push(tween({
      duration: D, ease: easeInOutQuad,
      update: function (p) { el.setAttribute(attr, lerpColor(ca, cb, p)) },
      complete: function () { el.setAttribute(attr, to); if (complete) complete() }
    }))
  }

  // line shapes paint with stroke
  if (a.shape === 'line' || b.shape === 'line') {
    if ((a.stroke || '') !== (b.stroke || '')) attrColorTween(target, 'stroke', a.stroke || 'none', b.stroke || 'none')
    return
  }
  var ag = a.fillGradient && a.fillGradient.stops && a.fillGradient.stops.length ? a.fillGradient : null
  var bg = b.fillGradient && b.fillGradient.stops && b.fillGradient.stops.length ? b.fillGradient : null
  if (!ag && !bg) {
    if (a.fill !== b.fill) attrColorTween(target, 'fill', a.fill || 'none', b.fill || 'none')
    return
  }
  ...
  var finals = bg ? bg.stops : ag.stops.map(function (s) { return { at: s.at, color: b.fill } })
  stops.forEach(function (st, i) {
    var at = finals[i] ? finals[i].at : 1
    var fromC = ag ? sampleGradient(ag.stops, at) : rgbaStr(colorParts(a.fill) || [0, 0, 0, 0])
    var toC = finals[i] ? finals[i].color : b.fill
    attrColorTween(st, 'stop-color', fromC, toC, i === 0 && fabricated ? function () {
  ...
  if (fromA !== toA) {
    this.tweens.push(tween({
      duration: D, ease: easeInOutQuad,
      update: function (p) { lin.setAttribute('gradientTransform', 'rotate(' + (fromA + (toA - fromA) * p) + ' 0.5 0.5)') }
    }))
  }
}
...
Presenter.prototype.nextIdx = function () {
  ...
  var anchor = this.idx
  if (cur.stateOf) {
    var p = slides.findIndex(function (s) { return s.id === cur.stateOf })
    if (p >= 0) anchor = p
  }
  for (var i = anchor + 1; i < slides.length; i++) if (!slides[i].stateOf) return i
  return -1
}
...
Presenter.prototype.prevIdx = function () {
  ...
  for (var i = this.idx - 1; i >= 0; i--) if (!slides[i].stateOf) return i
  return -1
}

Presenter.prototype.next = function () { var i = this.nextIdx(); if (i >= 0) this.show(i, 1) }
Presenter.prototype.prev = function () { var i = this.prevIdx(); if (i >= 0) this.show(i, -1) }

Presenter.prototype.onKey = function (e) {
  if (!this.overlay) return
  var k = e.key
  if (k === 'ArrowRight' || k === 'ArrowDown' || k === ' ' || k === 'PageDown' || k === 'Enter') { e.preventDefault(); e.stopPropagation(); this.next() }
  else if (k === 'ArrowLeft' || k === 'ArrowUp' || k === 'PageUp') { e.preventDefault(); e.stopPropagation(); this.prev() }
  else if (k === 'Home') { e.preventDefault(); this.show(0, -1) }
  else if (k === 'End') { e.preventDefault(); var lin = this.linear(); this.show(this.doc.slides.indexOf(lin[lin.length - 1]), 1) }
  else if (k === 's' || k === 'S') { e.preventDefault(); this.toggleSpeaker() }
  else if (k === 'b' || k === 'B' || k === '.') { e.preventDefault(); this.toggleBlack() }
  else if (k === 'Escape') { e.preventDefault(); e.stopPropagation(); this.close() }
}

Presenter.prototype.onClick = function (e) {
  var node = e.target.closest ? e.target.closest('[data-el-id]') : null
  if (node && this.slide) {
    var el = this.slide.elements.find(function (x) { return x.id === node.getAttribute('data-el-id') })
    if (el && el.link) {
      var t = this.doc.slides.findIndex(function (s) { return s.id === el.link })
      if (t >= 0) { this.show(t, 1); return }
    }
    if (el && (el.type === 'media')) return // let media controls work
  }
  this.next()
}
...
function Viewer(ppt, root) {
  ...
  var self = this
  root.querySelector('[data-v="prev"]').addEventListener('click', function () { self.go(-1) })
  root.querySelector('[data-v="next"]').addEventListener('click', function () { self.go(1) })
...
Viewer.prototype.go = function (d) {
  var lin = this.doc.slides.filter(function (s) { return !s.stateOf })
...
Viewer.prototype.render = function () {
  ...
  var w = this.doc.size.width, h = this.doc.size.height
  var s = Math.min((this.wrap.clientWidth - 16) / w, (this.wrap.clientHeight - 16) / h)
...
Viewer.prototype.destroy = function () { if (this._ro) this._ro.disconnect() }
/* --------------------------------------------------------------- editor */
...
Editor.prototype.slide = function () { return this.doc.slides[this.current] }

Editor.prototype.fit = function () {
  var w = this.doc.size.width, h = this.doc.size.height
  var aw = this.canvasWrap.clientWidth - 32, ah = this.canvasWrap.clientHeight - 32
...
Editor.prototype.zoomIn = function () { this.setZoom(this.zoom * 1.25) }
Editor.prototype.zoomOut = function () { this.setZoom(this.zoom / 1.25) }
Editor.prototype.zoomReset = function () { this.setZoom(1) }
...
Editor.prototype.renderThumbs = function () {
  ...
  doc.slides.forEach(function (slide, i) {
    ...
    t.addEventListener('dragleave', function () { t.classList.remove('ppt-drop') })
...
Editor.prototype.renderCanvas = function () {
  ...
  this.layer.addEventListener('pointerdown', function (e) { self.onPointerDown(e) })
  this.layer.addEventListener('dblclick', function (e) { self.onDblClick(e) })
  this.drawSelection()
  this.emitState()
}

Editor.prototype.findEl = function (id) {
  return this.slide().elements.find(function (el) { return el.id === id })
}
...
Editor.prototype.selEls = function () {
  var self = this
  return this.selIds.map(function (id) { return self.findEl(id) }).filter(Boolean)
}
...
Editor.prototype.drawSelection = function () {
  ...
  // prune dangling ids
  var self = this
  this.selIds = this.selIds.filter(function (id) { return !!self.findEl(id) })
  ...
  this.selIds.forEach(function (id, idx) {
    ...
    ov.style.cssText = 'left:' + el.x + 'px;top:' + el.y + 'px;width:' + el.w + 'px;height:' + el.h + 'px;' +
    ...
    if (single && el.type === 'shape' && el.shape === 'line') {
      ...
      ;['a', 'b'].forEach(function (w) {
        var d = document.createElement('i')
        d.className = 'ppt-lp' + (el[w === 'a' ? 'from' : 'to'] ? ' ppt-lp-linked' : '')
...
Editor.prototype.snapTargets = function () {
  ...
  var sel = {}
  this.selIds.forEach(function (id) { sel[id] = 1 })
...
function snapAxis(edges, targets, thr) {
  ...
  for (var i = 0; i < edges.length; i++) {
    for (var j = 0; j < targets.length; j++) {
      ...
      if (Math.abs(d) <= thr && (!best || Math.abs(d) < Math.abs(best.d))) best = { d: d, line: targets[j].v }
    }
  }
  return best
}
...
Editor.prototype.onPointerDown = function (e) {
  ...
  var self = this
  if (this.pen) { this.penPointerDown(e); return }
  if (this.pathEdit) { this.pathPointerDown(e); return }
  var rotNode = e.target.closest ? e.target.closest('.ppt-rot') : null
  if (rotNode && this.selIds.length === 1) { this.startRotate(e); return }
  var lpNode = e.target.closest ? e.target.closest('.ppt-lp') : null
  if (lpNode && this.selIds.length === 1) { this.startLinePoint(e, lpNode.getAttribute('data-ep')); return }
  var hNode = e.target.closest ? e.target.closest('.ppt-handle') : null
  if (hNode && this.selIds.length === 1) { this.startResize(e, hNode.getAttribute('data-h')); return }

  var elNode = e.target.closest ? e.target.closest('.ppt-el') : null
  if (!elNode) { this.startMarquee(e); return }
  ...
  var orig = els.map(function (el) { return { el: el, x: el.x, y: el.y, node: self.elNode(el.id) } })
  // selection bbox at drag start
  var bx = Math.min.apply(null, orig.map(function (o) { return o.x }))
  var by = Math.min.apply(null, orig.map(function (o) { return o.y }))
  var bw = Math.max.apply(null, orig.map(function (o) { return o.x + o.el.w })) - bx
  var bh = Math.max.apply(null, orig.map(function (o) { return o.y + o.el.h })) - by
  ...
  function move(ev) {
    ...
    var sx = snapAxis([bx + dx, bx + bw / 2 + dx, bx + bw + dx], targets.xs, thr)
    var sy = snapAxis([by + dy, by + bh / 2 + dy, by + bh + dy], targets.ys, thr)
    ...
    orig.forEach(function (o) {
      ...
      if (o.node) { o.node.style.left = o.el.x + 'px'; o.node.style.top = o.el.y + 'px' }
    })
    self.syncConnectors()
    self.drawSelection()
...
Editor.prototype.startMarquee = function (e) {
  ...
  function move(ev) {
    ...
    if (!moved && Math.abs(p.x - p0.x) + Math.abs(p.y - p0.y) < 3 / self.scale) return
    moved = true
    if (!mq) {
    ...
    var x = Math.min(p0.x, p.x), y = Math.min(p0.y, p.y)
    var w = Math.abs(p.x - p0.x), h = Math.abs(p.y - p0.y)
    mq.style.cssText = 'left:' + x + 'px;top:' + y + 'px;width:' + w + 'px;height:' + h + 'px;'
    // live-highlight hits
    var hits = []
    self.slide().elements.forEach(function (el) {
      if (el.x < x + w && el.x + el.w > x && el.y < y + h && el.y + el.h > y) hits.push(el.id)
    })
    self.selIds = base.concat(hits.filter(function (id) { return base.indexOf(id) < 0 }))
    self.drawSelection()
  }
  function up(ev) {
    window.removeEventListener('pointermove', move)
...
Editor.prototype.onDblClick = function (e) {
  if (this.pen) { this.finishPen(false); return }
  if (this.pathEdit) { this.pathDblClick(e); return }
  var elNode = e.target.closest ? e.target.closest('.ppt-el') : null
  if (!elNode) return
  var id = elNode.getAttribute('data-el-id')
  ...
  if (!el) return
  e.preventDefault()
  if (el.type === 'table') { this.startTableEdit(el, elNode); return }
  if (el.type === 'shape' && el.shape === 'path') { this.startPathEdit(el); return }
...
Editor.prototype.snapshot = function () {
  return JSON.stringify({
    slides: this.doc.slides, theme: this.doc.theme, size: this.doc.size, title: this.doc.title
  })
}
...
Editor.prototype.restore = function (json) {
  ...
  if (this.current >= this.doc.slides.length) this.current = this.doc.slides.length - 1
  this.selIds = []
  this.renderAll()
  this.syncConnectors()
  this.ppt.emit('change', { doc: this.ppt.getDoc() })
...
Editor.prototype.insertImage = function (file) {
  ...
  var img = new Image()
  img.onload = function () {
    var nw = img.naturalWidth || 100, nh = img.naturalHeight || 100
    ...
    var place = function (src) {
      ...
      var el = { id: uid(), type: 'image', src: src, fit: 'contain', radius: 0,
        x: Math.round((doc.size.width - w) / 2), y: Math.round((doc.size.height - h) / 2),
...
Editor.prototype.deleteSelected = function () {
  ...
  var self = this
  var ids = {}
  this.selIds.forEach(function (id) { ids[id] = 1 })
  this.beginChange()
  var removed = 0
  slide.elements = slide.elements.filter(function (el) {
    if (ids[el.id]) { self.notifyImageRemoved(el); removed++; return false }
    return true
  })
  if (!removed) { this._before = null; return false }
  this.selIds = []
  this.renderCanvas()
...
Editor.prototype.eachSel = function (type, fn) {
  var els = this.selEls().filter(function (el) { return !type || el.type === type })
  if (!els.length) return false
  this.beginChange()
  var self = this
  els.forEach(function (el) { fn(el); self.refreshEl(el) })
  this.commit()
  return true
}

Editor.prototype.setFill = function (color) {
  this.eachSel('shape', function (el) { el.fill = color; el.fillGradient = undefined })
}

Editor.prototype.setTextColor = function (color) {
  this.eachSel('text', function (el) { el.color = color; el.colorGradient = undefined })
}

Editor.prototype.setFontSize = function (n) {
  var v = clamp(parseInt(n, 10) || 32, 8, 400)
  this.eachSel('text', function (el) { el.fontSize = v })
}

Editor.prototype.toggleBold = function () {
  this.eachSel('text', function (el) { el.fontWeight = el.fontWeight >= 700 ? 400 : 700 })
}

Editor.prototype.setAlign = function (a) {
  this.eachSel('text', function (el) { el.align = a })
}
...
Editor.prototype.deleteSlide = function () {
  ...
  ;(gone.elements || []).forEach(function (el) { self.notifyImageRemoved(el) })
...
Editor.prototype.addElement = function (kind) {
  var doc = this.doc
  var theme = doc.theme
  var W = doc.size.width, H = doc.size.height
  var el
  if (kind === 'text') {
    el = { id: uid(), type: 'text', x: W / 2 - 160, y: H / 2 - 40, w: 320, h: 80, rotation: 0, opacity: 1,
      html: 'Text', fontSize: 32, fontFamily: theme.fontFamily, fontWeight: 400,
      color: theme.color, align: 'center', valign: 'middle', lineHeight: 1.25 }
  } else if (kind === 'line') {
    el = { id: uid(), type: 'shape', shape: 'line', x: W / 2 - 120, y: H / 2 - 10, w: 240, h: 20, rotation: 0,
      opacity: 1, fill: 'none', stroke: theme.color, strokeWidth: 3, radius: 0, lineEnd: 'none' }
  } else {
    el = { id: uid(), type: 'shape', shape: kind, x: W / 2 - 100, y: H / 2 - 60, w: 200, h: 120, rotation: 0,
      opacity: 1, fill: theme.accent, stroke: '', strokeWidth: 0, radius: kind === 'rect' ? 8 : 0 }
  }
...
Editor.prototype.insertTable = function () {
  var doc = this.doc, theme = doc.theme
  this.beginChange()
  var st = Object.assign({
    headerBg: theme.accent, headerColor: '#ffffff', zebra: 'rgba(128,128,128,.08)',
    borderColor: 'rgba(128,128,128,.3)', borderWidth: 1, cellPadX: 10, cellPadY: 8,
    fontSize: 15, color: theme.color, radius: 6
  }, theme.table || {})
  var el = {
    id: uid(), type: 'table',
    x: Math.round(doc.size.width / 2 - 260), y: Math.round(doc.size.height / 2 - 110),
    w: 520, h: 220, rotation: 0, opacity: 1,
    columns: [{ w: 1 }, { w: 1 }, { w: 1 }, { w: 1 }],
    rows: [
      { cells: [{ html: 'Name' }, { html: 'Q1' }, { html: 'Q2' }, { html: 'Q3' }] },
      { cells: [{ html: 'Alpha' }, { html: '12' }, { html: '18' }, { html: '27' }] },
      { cells: [{ html: 'Beta' }, { html: '8' }, { html: '11' }, { html: '16' }] }
    ],
    header: true, style: st
  }
  this.slide().elements.push(el)
  this.renderCanvas()
  this.select(el.id)
...
Editor.prototype.insertChart = function (preset) {
  ...
  var option
  if (preset === 'pie') {
    option = { series: [{ type: 'pie', name: 'Share', data: [{ name: 'A', value: 42 }, { name: 'B', value: 28 }, { name: 'C', value: 18 }, { name: 'D', value: 12 }] }] }
  } else if (preset === 'line' || preset === 'scatter') {
    option = { xAxis: { data: ['W1', 'W2', 'W3', 'W4', 'W5', 'W6'] }, series: [
      { type: preset, name: 'Series 1', data: [12, 19, 15, 24, 22, 31] },
      { type: preset, name: 'Series 2', data: [8, 11, 13, 17, 19, 24] }] }
  } else {
    preset = 'bar'
    option = { xAxis: { data: ['Q1', 'Q2', 'Q3', 'Q4'] }, series: [
      { type: 'bar', name: 'Series 1', data: [12, 19, 15, 24] },
      { type: 'bar', name: 'Series 2', data: [8, 11, 13, 17] }] }
  }
  var el = { id: uid(), type: 'chart', preset: preset, option: option,
    x: Math.round(doc.size.width / 2 - 280), y: Math.round(doc.size.height / 2 - 170),
    w: 560, h: 340, rotation: 0, opacity: 1 }
  this.slide().elements.push(el)
  this.renderCanvas()
  this.select(el.id)
  this.commit()
...
Editor.prototype.reseedChart = function (preset) {
  ...
  if (preset === 'pie') {
    ...
    el.option = { series: [{ type: 'pie', name: (series[0] && series[0].name) || 'Share',
      data: vals.map(function (v, i) { return { name: cats[i] != null ? String(cats[i]) : (v && v.name) || 'S' + (i + 1), value: typeof v === 'object' && v !== null ? +v.value || 0 : +v || 0 } }) }] }
  } else {
    if (!cats.length) cats = ['Q1', 'Q2', 'Q3', 'Q4']
    el.option = { xAxis: { data: cats }, series: (series.length ? series : [{ name: 'Series 1', data: [] }]).map(function (s) {
      return { type: preset, name: s.name || 'Series', data: (s.data || []).map(function (v) { return typeof v === 'object' && v !== null ? +v.value || 0 : +v || 0 }) }
    }) }
  }
  el.preset = preset
  this.refreshEl(el)
...
Editor.prototype.insertMedia = function (kind, file) {
  ...
  var place = function (src) {
    var doc = self.doc
    var w = kind === 'audio' ? 340 : 480, h = kind === 'audio' ? 54 : 270
    var el = { id: uid(), type: 'media', kind: kind, src: src,
      x: Math.round((doc.size.width - w) / 2), y: Math.round((doc.size.height - h) / 2),
      w: w, h: h, rotation: 0, opacity: 1, controls: true }
...
Editor.prototype.insertSvg = function (markup) {
  ...
  if (!markup || markup.indexOf('<svg') < 0) { this.ppt.emit('error', { message: 'Not SVG markup' }); return }
  this.beginChange()
  var doc = this.doc
  var el = { id: uid(), type: 'svg', markup: markup,
    x: Math.round(doc.size.width / 2 - 120), y: Math.round(doc.size.height / 2 - 120),
    w: 240, h: 240, rotation: 0, opacity: 1 }
  this.slide().elements.push(el)
  this.renderCanvas()
  this.select(el.id)
  this.commit()
...
Editor.prototype.copySelection = function (cut) {
  var els = this.selEls()
  if (!els.length) return false
  var payload = { __ppt: 'clip', elements: els.map(function (el) { return deepClone(el) }) }
  try { navigator.clipboard.writeText(JSON.stringify(payload)).catch(function () {}) } catch (e) {}
...
Editor.prototype.duplicateSelected = function () {
  ...
  els.forEach(function (el) {
    ...
    cp.x = (+cp.x || 0) + 12
    cp.y = (+cp.y || 0) + 12
    delete cp.from; delete cp.to // connector anchors would be stale
...
Editor.prototype.pastePayload = function (payload) {
  if (!payload || !Array.isArray(payload.elements) || !payload.elements.length) return
  this.beginChange()
  var self = this, newIds = []
...
Editor.prototype.bindPaste = function () {
  ...
  this._onPaste = function (e) {
    ...
    try {
      ...
      if (j && j.__ppt === 'clip' && Array.isArray(j.elements)) { self.pastePayload(j); return }
    } catch (err) {}
    self.beginChange()
    var doc = self.doc
    var el = { id: uid(), type: 'text',
      x: Math.round(doc.size.width / 2 - 200), y: Math.round(doc.size.height / 2 - 40),
      w: 400, h: 80, rotation: 0, opacity: 1,
      html: escapeHtml(text.slice(0, 500)).replace(/\r?\n/g, '<br>'),
      fontSize: 24, fontFamily: doc.theme.fontFamily, fontWeight: 400,
      color: doc.theme.color, align: 'left', valign: 'top', lineHeight: 1.3 }
    self.slide().elements.push(el)
    self.renderCanvas()
    self.select(el.id)
    self.commit()
...
function anchorPoint(el, side, toward) {
  ...
  var t = Math.min(dx ? (el.w / 2) / Math.abs(dx) : Infinity, dy ? (el.h / 2) / Math.abs(dy) : Infinity)
...
Editor.prototype.syncConnectors = function () {
  ...
  if (!slide) return
  var byId = {}
  slide.elements.forEach(function (el) { byId[el.id] = el })
  var self = this
  slide.elements.forEach(function (el) {
    ...
    if (!el.from && !el.to) { self.refreshEl(el); return }
    var ep = lineEndpoints(el)
...
var bezLerp = function (a, b, t) { return { x: a.x + (b.x - a.x) * t, y: a.y + (b.y - a.y) * t } }
var bezR = function (v) { return Math.round(v * 100) / 100 }
var bezDist = function (a, b) { var dx = a.x - b.x, dy = a.y - b.y; return Math.sqrt(dx * dx + dy * dy) }
...
function dToBezier(d) {
  ...
  var nodes = []
  var closed = false
  var i = 0, cmd = '', px = 0, py = 0, sx = 0, sy = 0
  var num = function () { return Number(tokens[i++]) }
  function pushNode(x, y, c1, c2) {
    var prev = nodes[nodes.length - 1]
    var p = { x: x, y: y }
    var node = { p: p, in: c2 || (prev ? bezLerp(prev.p, p, 2 / 3) : undefined), out: undefined, corner: false }
  ...
  while (i < tokens.length) {
    var tk = tokens[i]
    if (/^[a-zA-Z]$/.test(tk)) { cmd = tk; i++; if (/z/i.test(cmd)) { closed = true; px = sx; py = sy } continue }
    var rel = cmd >= 'a' && cmd <= 'z'
    var C = cmd.toUpperCase()
    if (C === 'M') {
      var mx = num() + (rel ? px : 0), my = num() + (rel ? py : 0)
      ...
      var rc1 = prevS && prevS.out ? { x: 2 * px - prevS.out.x, y: 2 * py - prevS.out.y } : { x: px, y: py }
...
function bezierToD(nodes, closed) {
  ...
  for (var i = 0; i < segs; i++) {
    ...
    var c1 = a.out || a.p, c2 = b.in || b.p
    d += 'C' + bezR(c1.x) + ' ' + bezR(c1.y) + ' ' + bezR(c2.x) + ' ' + bezR(c2.y) + ' ' + bezR(b.p.x) + ' ' + bezR(b.p.y)
  }
  if (closed) d += 'Z'
  return d
}
...
function splitCubic(p0, c1, c2, p1, t) {
  var q0 = bezLerp(p0, c1, t), q1 = bezLerp(c1, c2, t), q2 = bezLerp(c2, p1, t)
  var r0 = bezLerp(q0, q1, t), r1 = bezLerp(q1, q2, t)
...
function bezierBBox(nodes) {
  ...
  nodes.forEach(function (nd) {
    xs.push(nd.p.x); ys.push(nd.p.y)
    if (nd.in) { xs.push(nd.in.x); ys.push(nd.in.y) }
    if (nd.out) { xs.push(nd.out.x); ys.push(nd.out.y) }
  })
...
Editor.prototype.cancelPen = function () {
  ...
  if (this._penMove && this.layer) this.layer.removeEventListener('pointermove', this._penMove)
  this._penMove = null
...
Editor.prototype.penPointerDown = function (e) {
  ...
  function move(ev) {
    ...
    var dx = q.x - nd.p.x, dy = q.y - nd.p.y
  ...
  if (this.pen) {
    ...
    if (nodes.length) {
      ...
      if (this.pen.cur) {
        ...
        d = bezierToD(nodes.concat([{ p: { x: c.x, y: c.y }, in: bezLerp(last.p, c, 2 / 3), corner: false }]), false)
      }
      ...
      nodes.forEach(function (nd, i) {
        ...
        if (nd.bent) {
          if (nd.in) {
            el('line', { x1: nd.p.x, y1: nd.p.y, x2: nd.in.x, y2: nd.in.y }, 'ppt-pen-hl')
            el('circle', { cx: nd.in.x, cy: nd.in.y, r: 3 }, 'ppt-pen-h')
          }
          if (nd.out) {
            el('line', { x1: nd.p.x, y1: nd.p.y, x2: nd.out.x, y2: nd.out.y }, 'ppt-pen-hl')
            el('circle', { cx: nd.out.x, cy: nd.out.y, r: 3 }, 'ppt-pen-h')
          }
        }
        el('rect', { x: nd.p.x - 3.5, y: nd.p.y - 3.5, width: 7, height: 7 }, i === 0 && nodes.length >= 3 ? 'ppt-pen-a ppt-pen-first' : 'ppt-pen-a')
      })
    }
  } else if (this.pathEdit) {
    var pe = this.pathEdit
    var el2 = this.findEl(pe.id)
    el('path', { d: bezierToD(pe.nodes, pe.closed), style: el2 ? 'fill:' + (el2.fill || 'none') + ';stroke:' + (el2.stroke || 'none') + ';stroke-width:' + (el2.strokeWidth || 1) : '' }, 'ppt-pen-live')
    pe.nodes.forEach(function (nd, i) {
      if (nd.in) {
        el('line', { x1: nd.p.x, y1: nd.p.y, x2: nd.in.x, y2: nd.in.y }, 'ppt-pen-hl')
        el('circle', { cx: nd.in.x, cy: nd.in.y, r: 4, 'data-i': i, 'data-h': 'in' }, 'ppt-bh')
      }
      if (nd.out) {
        el('line', { x1: nd.p.x, y1: nd.p.y, x2: nd.out.x, y2: nd.out.y }, 'ppt-pen-hl')
        el('circle', { cx: nd.out.x, cy: nd.out.y, r: 4, 'data-i': i, 'data-h': 'out' }, 'ppt-bh')
      }
      el('rect', { x: nd.p.x - 4, y: nd.p.y - 4, width: 8, height: 8, 'data-i': i }, 'ppt-ba')
    })
  }
...
Editor.prototype.startPathEdit = function (el) {
  ...
  var sx = el.w / Math.max(pb[2], 1), sy = el.h / Math.max(pb[3], 1)
  var ox = el.x + pb[0], oy = el.y + pb[1]
  var nodes = parsed.nodes.map(function (nd) {
    var n = { p: { x: ox + nd.p.x * sx, y: oy + nd.p.y * sy }, corner: nd.corner }
...
Editor.prototype.finishPathEdit = function () {
  ...
  if (!el) { this._before = null; return }
...
Editor.prototype.pathPointerDown = function (e) {
  ...
  if (bh) {
    e.preventDefault()
    var iH = +bh.getAttribute('data-i'), which = bh.getAttribute('data-h')
    var nd = pe.nodes[iH]
    function moveH(ev) {
      var q = self.slidePoint(ev)
  ...
  if (ba) {
    ...
    function moveA(ev) {
      ...
      if (pull) {
        ...
        var dx = q.x - ndA.p.x, dy = q.y - ndA.p.y
        ndA.p = { x: q.x, y: q.y }
...
Editor.prototype.pathInsertAt = function (p) {
  ...
  for (var i = 0; i < segs; i++) {
    var a = pe.nodes[i], b = pe.nodes[(i + 1) % n]
    var c1 = a.out || a.p, c2 = b.in || b.p
    for (var s = 0; s <= 24; s++) {
      var t = s / 24
  ...
  var a2 = pe.nodes[best.i], b2 = pe.nodes[(best.i + 1) % n]
  var sp = splitCubic(a2.p, a2.out || a2.p, b2.in || b2.p, b2.p, best.t)
  a2.out = sp.c1l
  b2.in = sp.c2r
  pe.nodes.splice(best.i + 1, 0, { p: sp.mid, in: sp.c2l, out: sp.c1r, corner: false })
  this.drawPen()
  return true
}
...
Editor.prototype.bindKeys = function () {
  ...
  this._onKey = function (e) {
    ...
    if (mod && (e.key.toLowerCase() === 'y' || (e.key.toLowerCase() === 'z' && e.shiftKey))) { e.preventDefault(); self.redo(); return }
    // zoom: Cmd/Ctrl + = / - / 0
    if (mod && (e.key === '=' || e.key === '+')) { e.preventDefault(); self.zoomIn(); return }
    if (mod && e.key === '-') { e.preventDefault(); self.zoomOut(); return }
    if (mod && e.key === '0') { e.preventDefault(); self.zoomReset(); return }
    // F5: present from current slide
    if (e.key === 'F5') { e.preventDefault(); self.ppt.present(self.current); return }
    // [ / ]: previous / next slide
    if (e.key === '[') { e.preventDefault(); self.goTo(-1); return }
    if (e.key === ']') { e.preventDefault(); self.goTo(1); return }
    // pen / path-edit modes: Enter finishes, Escape cancels
    if (self.pen) {
      if (e.key === 'Enter') { e.preventDefault(); self.finishPen(false) }
      else if (e.key === 'Escape') { e.preventDefault(); self.cancelPen() }
      return
    }
    if (self.pathEdit) {
      if (e.key === 'Enter') { e.preventDefault(); self.finishPathEdit() }
      else if (e.key === 'Escape') { e.preventDefault(); self.cancelPathEdit() }
      return
    }
    // Escape: clear selection
    if (e.key === 'Escape') {
      if (self.selIds.length) { e.preventDefault(); self.select(null) }
      return
    }
    if (!self.selIds.length) return
    // clipboard: Ctrl+C / X / D (paste is a native 'paste' event — see bindPaste)
    if (mod && e.key.toLowerCase() === 'c') { e.preventDefault(); self.copySelection(false); return }
    if (mod && e.key.toLowerCase() === 'x') { e.preventDefault(); self.copySelection(true); return }
    if (mod && e.key.toLowerCase() === 'd') { e.preventDefault(); self.duplicateSelected(); return }
    if (e.key === 'Delete' || e.key === 'Backspace') { e.preventDefault(); self.deleteSelected(); return }
    var step = e.shiftKey ? 10 : 1
    var d = { ArrowLeft: [-step, 0], ArrowRight: [step, 0], ArrowUp: [0, -step], ArrowDown: [0, step] }[e.key]
    if (d) {
      e.preventDefault()
      if (!self._nudging) { self._nudging = true; self.beginChange() }
      ...
      self._nudgeT = setTimeout(function () { self._nudging = false; self.commit() }, 400)
    }
  }
...
PPT.prototype.getDoc = function () { return deepClone(this.doc) }
...
PPT.prototype.emit = function (evt, payload) {
  ...
  for (var i = 0; i < list.length; i++) {
    try { list[i](payload) } catch (e) { setTimeout(function () { throw e }, 0) }
  }
...
PPT.prototype.getState = function () {
  ...
  if (ed) {
    ...
    if (slide) {
      ...
      st.slide = { id: slide.id, name: slide.name || '', stateOf: slide.stateOf || '', notes: slide.notes || '' }
    }
    var els = ed.selEls()
    if (els.length) {
      ...
      seln._ids = els.map(function (x) { return x.id })
      st.selection = seln
    }
  } else if (this.viewer) {
...
PPT.prototype.insertText = function () { if (this.editor) this.editor.insertText(); return this }
PPT.prototype.insertShape = function (kind) { if (this.editor) this.editor.insertShape(kind); return this }
PPT.prototype.insertImage = function (file) { if (this.editor) this.editor.insertImage(file); return this }
PPT.prototype.deleteSelected = function () { if (this.editor) this.editor.deleteSelected(); return this }
PPT.prototype.setFill = function (color) { if (this.editor) this.editor.setFill(color); return this }
PPT.prototype.setTextColor = function (color) { if (this.editor) this.editor.setTextColor(color); return this }
PPT.prototype.setFontSize = function (n) { if (this.editor) this.editor.setFontSize(n); return this }
PPT.prototype.toggleBold = function () { if (this.editor) this.editor.toggleBold(); return this }
PPT.prototype.setAlign = function (a) { if (this.editor) this.editor.setAlign(a); return this }
PPT.prototype.setSlideBackground = function (color) { if (this.editor) this.editor.setSlideBackground(color); return this }
PPT.prototype.setTransition = function (t) { if (this.editor) this.editor.setTransition(t); return this }
PPT.prototype.addSlide = function () { if (this.editor) this.editor.addSlide(); return this }
PPT.prototype.duplicateSlide = function () { if (this.editor) this.editor.duplicateSlide(); return this }
PPT.prototype.deleteSlide = function () { if (this.editor) this.editor.deleteSlide(); return this }
PPT.prototype.moveSlide = function (d) { if (this.editor) this.editor.moveSlide(d); return this }

PPT.prototype.undo = function () { if (this.editor) this.editor.undo(); return this }
PPT.prototype.redo = function () { if (this.editor) this.editor.redo(); return this }
PPT.prototype.insertTable = function () { if (this.editor) this.editor.insertTable(); return this }
PPT.prototype.insertChart = function (preset) { if (this.editor) this.editor.insertChart(preset); return this }
PPT.prototype.insertMedia = function (kind, file) { if (this.editor) this.editor.insertMedia(kind, file); return this }
PPT.prototype.insertSvg = function (markup) { if (this.editor) this.editor.insertSvg(markup); return this }
PPT.prototype.reseedChart = function (preset) { if (this.editor) this.editor.reseedChart(preset); return this }
PPT.prototype.duplicateSelected = function () { if (this.editor) this.editor.duplicateSelected(); return this }
PPT.prototype.copySelection = function (cut) { if (this.editor) this.editor.copySelection(cut); return this }
PPT.prototype.moveSlideTo = function (from, to) { if (this.editor) this.editor.moveSlideTo(from, to); return this }
PPT.prototype.insertSlideAt = function (i) { if (this.editor) this.editor.insertSlideAt(i); return this }
PPT.prototype.togglePen = function () { if (this.editor) this.editor.togglePen(); return this }
...
PPT.prototype.print = function () {
  ...
  if (doc.fonts && doc.fonts.length) {
    fontsCss = doc.fonts.map(function (f) {
      var src = resolveAsset(doc, 'asset:' + f.asset)
      if (!src) return ''
      return '@font-face{font-family:"' + f.family + '";src:url(' + src + ');' +
        (f.weight ? 'font-weight:' + f.weight + ';' : '') + (f.style ? 'font-style:' + f.style + ';' : '') + '}'
  ...
  d.open()
  d.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + escapeHtml(doc.title) + '</title><style>' +
    '@page{size:' + doc.size.width + 'px ' + doc.size.height + 'px;margin:0;}' +
  ...
  d.close()
  doc.slides.forEach(function (s) {
    if (s.stateOf) return // interactive states are skipped in linear print
...
PPT.prototype.updateSelected = function (props) { if (this.editor) this.editor.updateSelected(props); return this }
PPT.prototype.updateSlide = function (props) { if (this.editor) this.editor.updateSlide(props); return this }
PPT.prototype.updateDocSize = function (w, h) { if (this.editor) this.editor.updateDocSize(w, h); return this }
PPT.prototype.goTo = function (d) { if (this.editor) this.editor.goTo(d); return this }
PPT.prototype.setZoom = function (z) { if (this.editor) this.editor.setZoom(z); return this }
PPT.prototype.zoomIn = function () { if (this.editor) this.editor.zoomIn(); return this }
PPT.prototype.zoomOut = function () { if (this.editor) this.editor.zoomOut(); return this }
PPT.prototype.zoomReset = function () { if (this.editor) this.editor.zoomReset(); return this }
